...
class RealExecutionManager:
    """REAL validation layer with fail-closed safety.

    In 0.12.1 all state-changing broker POST calls are hard-disabled.
    ...
    def execute_buy(self, prepared: dict, confirmation: str, *, auto: bool = False) -> dict:
        raise RealExecutionError(
            "REAL POST ist in TradePilot 0.12.1 hart deaktiviert. "
            "Nutze 17_TEST_REAL_ROUNDTRIP_VALIDATION_NO_POST.bat für die Roundtrip-Validierung."
        )
    ...
    def close_position(self, position_id: str, confirmation: str, *, auto: bool = False) -> dict:
        raise RealExecutionError(
            "REAL CLOSE POST ist in TradePilot 0.12.1 hart deaktiviert. "
            "Nutze 17_TEST_REAL_ROUNDTRIP_VALIDATION_NO_POST.bat für die Roundtrip-Validierung."
        )
