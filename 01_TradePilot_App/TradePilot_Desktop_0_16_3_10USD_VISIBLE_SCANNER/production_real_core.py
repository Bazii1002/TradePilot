...
The preferred live validation amount is USD 10.00. The core never auto-increases a
...
REAL_TEST_USD = 10.00
MAX_REAL_TRADE_USD = 10.00
MAX_REAL_POSITIONS = 1
MAX_TRADES_PER_DAY = 3
MAX_INVESTED_CAPITAL_USD = 10.00
MAX_DAILY_LOSS_USD = 20.00
# Backward-compatible aliases for older diagnostic modules only.
REAL_TEST_EUR = REAL_TEST_USD
MAX_REAL_TRADE_EUR = MAX_REAL_TRADE_USD
MAX_INVESTED_CAPITAL_EUR = MAX_INVESTED_CAPITAL_USD
MAX_DAILY_LOSS_EUR = MAX_DAILY_LOSS_USD
LEVERAGE = 1
...
@dataclass
class RiskLimits:
    max_trade_usd: float = MAX_REAL_TRADE_USD
    max_open_positions: int = MAX_REAL_POSITIONS
    max_trades_per_day: int = MAX_TRADES_PER_DAY
    max_invested_capital_usd: float = MAX_INVESTED_CAPITAL_USD
...
class RiskManager:
    ...
    def validate_new_buy(
        ...
        *,
        ...
        if amount <= 0 or amount > L.max_trade_usd:
            reasons.append(f"amount must be >0 and <= USD {L.max_trade_usd:.2f}")
        if int(open_positions) >= L.max_open_positions:
            reasons.append("max REAL positions reached")
        if float(invested_usd) + amount > L.max_invested_capital_usd + 1e-9:
        ...
        if float(realized_pnl_today_usd) <= -abs(L.max_daily_loss_usd):
            reasons.append("daily loss gate reached")
        return {"ok": not reasons, "reasons": reasons, "amount_usd": amount, "limits": asdict(L)}

    def broker_minimum_policy(self, requested_usd: float, broker_minimum_usd: float | None) -> dict[str, Any]:
        req = round(float(requested_usd), 2)
        minimum = None if broker_minimum_usd is None else round(float(broker_minimum_usd), 2)
        if minimum is not None and minimum > req:
            return {
                "ok": False,
                "requested_usd": req,
                "broker_minimum_usd": minimum,
                "auto_increase": False,
                "reason": "Broker minimum is higher; TradePilot blocks instead of increasing the order.",
            }
        return {"ok": True, "requested_usd": req, "broker_minimum_usd": minimum, "auto_increase": False}
...
class ProductionRealCore:
    ...
    def status(self) -> dict[str, Any]:
        return {
            "version": "0.16.0",
            "real_auto_enabled": False,
            "preferred_test_usd": REAL_TEST_USD,
