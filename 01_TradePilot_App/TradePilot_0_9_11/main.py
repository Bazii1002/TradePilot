...
from etoro_broker import EtoroDemoBroker, EtoroError
...
class BotPage(QWidget):
    ...
    def __init__(self, config: dict, broker: PaperBroker, parent=None):
        ...
        self.broker = broker
        self.etoro = EtoroDemoBroker(APP_DIR)
...
class SettingsPage(QWidget):
    ...
    def __init__(self, config: dict, broker: PaperBroker, parent=None):
        ...
        warn = QLabel(loc(
            ...
            "Automatische Broker-Orders bleiben in 0.9.11 deaktiviert; nur bestätigte manuelle eToro-Demo-Tests sind möglich.",
            "Note: If AutoTrader is active, scheduled scans may open or close paper trades. "
            "Automatic broker orders remain disabled in 0.9.11; only confirmed manual eToro demo tests are available."))
        ...
        eth = QLabel(loc("eToro Demo API", "eToro Demo API")); eth.setObjectName("SectionTitle")
        ethelp = QLabel(loc(
            "0.9.11 verbindet TradePilot erstmals mit der offiziellen eToro-Demo-API. Zugangsdaten werden nur lokal in .env gespeichert. Echtgeld-Orders und AutoTrader→eToro sind in dieser Version absichtlich gesperrt.",
            "0.9.11 connects TradePilot to the official eToro Demo API for the first time. Credentials are stored only in the local .env file. Real-money orders and AutoTrader→eToro are intentionally disabled in this version."))
        ...
        userrow=QHBoxLayout(); userl=QLabel("User Key (Demo)"); userl.setMinimumWidth(180); self.etoro_user_key=QLineEdit(); self.etoro_user_key.setEchoMode(QLineEdit.EchoMode.Password); self.etoro_user_key.setPlaceholderText(loc("Demo User Key", "Demo User Key")); userrow.addWidget(userl); userrow.addStretch(1); userrow.addWidget(self.etoro_user_key); etl.addLayout(userrow)
        ...
        orderrow=QHBoxLayout(); syml=QLabel(loc("Manueller Demo-Test", "Manual demo test")); syml.setMinimumWidth(180); self.etoro_symbol=QLineEdit("AAPL"); self.etoro_symbol.setMaximumWidth(120); self.etoro_amount=QLineEdit("100"); self.etoro_amount.setMaximumWidth(120); orderbtn=QPushButton(loc("Demo BUY senden", "Send demo BUY")); orderbtn.setObjectName("Danger"); orderbtn.clicked.connect(self._etoro_manual_buy); orderrow.addWidget(syml); orderrow.addStretch(1); orderrow.addWidget(self.etoro_symbol); orderrow.addWidget(self.etoro_amount); orderrow.addWidget(orderbtn); etl.addLayout(orderrow)
        orderhint=QLabel(loc("Order wird erst nach einer zusätzlichen Bestätigung wirklich an das eToro-Demokonto gesendet. Harte Obergrenze in 0.9.11: 250 USD pro manuellem Test.", "The order is only sent to the eToro demo account after an additional confirmation. Hard limit in 0.9.11: USD 250 per manual test.")); orderhint.setObjectName("Muted"); orderhint.setWordWrap(True); etl.addWidget(orderhint)
        ...
        details = QLabel(
            ...
            "AutoTrader 0.9.11: eToro Demo API Verbindung + manueller Demo-Ordertest; AutoTrader→eToro weiterhin gesperrt"
        )
    ...
    def _etoro_save_keys(self):
        try:
            ...
            self.etoro_status.setText(loc("Keys lokal gespeichert (.env)", "Keys saved locally (.env)"))
    ...
    def _etoro_test_connection(self):
        try:
            ...
            self.etoro_status.setText(loc("Verbindung fehlgeschlagen", "Connection failed")); self.etoro_status.setStyleSheet(f"color:{RED};")
            QMessageBox.warning(self, "eToro Demo", str(exc))
    ...
    def _etoro_manual_buy(self):
        ...
        try:
            ...
            amount=0.0
        if not symbol or amount <= 0 or amount > 250:
            QMessageBox.warning(self,"eToro Demo",loc("Ticker angeben und Betrag zwischen 1 und 250 USD wählen.","Enter a ticker and choose an amount between USD 1 and 250.")); return
        try:
            self.etoro.save_credentials(self.etoro_api_key.text(), self.etoro_user_key.text())
        except Exception as exc:
            QMessageBox.warning(self,"eToro Demo",str(exc)); return
        ans=QMessageBox.question(self,"eToro DEMO",loc(
            f"Wirklich eine DEMO-Market-BUY-Order senden?\n\n{symbol} · {amount:.2f} USD\n\nDies betrifft ausschließlich das eToro Virtual/Demo Portfolio.",
            f"Really send a DEMO market BUY order?\n\n{symbol} · USD {amount:.2f}\n\nThis affects only the eToro Virtual/Demo portfolio."),QMessageBox.StandardButton.Yes|QMessageBox.StandardButton.No)
        if ans!=QMessageBox.StandardButton.Yes: return
        try:
            result=self.etoro.place_demo_market_buy(symbol, amount)
            response=result.get("response")
            QMessageBox.information(self,"eToro Demo",loc(f"Demo-Order wurde an eToro gesendet.\n\nInstrument-ID: {result.get('instrument_id')}\nAntwort: {response}",f"Demo order sent to eToro.\n\nInstrument ID: {result.get('instrument_id')}\nResponse: {response}"))
