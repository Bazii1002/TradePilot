...
def normalize_event(raw: dict) -> dict | None:
    scheduled = (raw.get("scheduledAt") or raw.get("scheduled_at") or raw.get("date") or raw.get("Date")
                 or raw.get("datetime") or raw.get("dateTime") or raw.get("timestamp") or raw.get("time"))
    dt = _parse_dt(scheduled)
    if not dt:
        return None
    name = (raw.get("eventName") or raw.get("event_name") or raw.get("name") or raw.get("Event")
            or raw.get("title") or raw.get("indicator") or "-")
    importance = (raw.get("importance") or raw.get("impact") or raw.get("Importance")
                  or raw.get("importanceLevel") or raw.get("volatility") or "-")
    relevance, semantic = classify_event(str(name), str(importance))
    country = (raw.get("country") or raw.get("Country") or raw.get("countryCode")
               or raw.get("currency") or raw.get("Currency") or "—")
    event = {
        "event_name": str(name),
        "country": str(country),
        ...
        "forecast": next((raw.get(k) for k in ("forecast", "Forecast", "consensus", "Consensus", "estimate") if raw.get(k) is not None), None),
        "previous": next((raw.get(k) for k in ("previous", "Previous", "prior", "Prior") if raw.get(k) is not None), None),
        "actual": next((raw.get(k) for k in ("actual", "Actual", "value", "Value", "ActualValue") if raw.get(k) is not None), None),
        "unit": raw.get("unit") or raw.get("Unit") or raw.get("currency") or raw.get("Currency"),
    }
...
def compute_macro_snapshot(events: list[dict], market: dict | None = None, now: datetime | None = None, data_ok: bool = True) -> dict:
    ...
    if not data_ok:
        ...
        # Released HIGH/CRITICAL events without Actual are release-data-pending.
        # Fail-closed: never resume/increase aggression until the release is observable.
        for e in recent:
            if e.get("relevance") in {"HIGH", "CRITICAL"} and e.get("actual") in (None, "", "-", "—"):
                risk, blocked, multiplier = "HIGH", True, 0.0
                reasons.append(f"{e['event_name']}: Release-Daten noch ausstehend → neue Trades pausiert")
