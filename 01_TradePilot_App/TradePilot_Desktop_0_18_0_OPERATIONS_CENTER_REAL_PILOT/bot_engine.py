...
class BotEngine(QObject):
    ...
    def __init__(self, app_dir: Path, macro_engine=None):
        ...
        self._last_actionable = None
        self._last_decision_reason = "Noch keine Bot-Entscheidung"
        self._next_scan_at = None
        self._last_state_save = "—"
        self._load_state()
        ...
        # Only SHADOW scanning may resume automatically. REAL AUTO remains hard locked.
        if self._resume_requested:
            QTimer.singleShot(900, self._resume_shadow_after_restart)
    ...
    def _save_state(self):
        payload = {
            ...
            "was_running": self._running, "last_scan": self._last_scan, "last_scan_summary": self._last_scan_summary,
            "last_actionable": self._last_actionable, "last_decision_reason": self._last_decision_reason,
        }
        try:
            tmp = self.state_file.with_suffix(".tmp")
            tmp.write_text(json.dumps(payload, indent=2), encoding="utf-8")
            tmp.replace(self.state_file)
            self._last_state_save = datetime.now().strftime("%H:%M:%S")
        except Exception:
            pass
    ...
    def _load_state(self):
        ...
        try:
            ...
            self._resume_requested = bool(p.get("was_running", False))
            self._last_scan = str(p.get("last_scan") or self._last_scan)
            self._last_scan_summary = str(p.get("last_scan_summary") or self._last_scan_summary)
            self._last_actionable = p.get("last_actionable") if isinstance(p.get("last_actionable"), dict) else None
            self._last_decision_reason = str(p.get("last_decision_reason") or self._last_decision_reason)
        except Exception:
            pass

    def _heartbeat_tick(self):
        if self._running and self._next_scan_at:
            remain = max(0, int((self._next_scan_at - datetime.now()).total_seconds()))
            self._next_scan = f"in {remain} Sek."
        self.changed.emit()

    def _resume_shadow_after_restart(self):
        if not self._resume_requested or self._running:
            return
        self._resume_requested = False
        self._log("SHADOW nach App-Neustart sicher fortgesetzt · REAL AUTO bleibt LOCKED")
        self.startBot()

    @Property(bool, notify=changed)
    def resumePending(self): return self._resume_requested
    @Property(str, notify=changed)
    def restartRecoveryText(self):
        if self._running and self._scan_count > 0:
            return "STATE RESTORED · SHADOW RUNNING"
        if self._resume_requested:
            return "STATE RESTORED · RESUME PENDING"
        return "STATE PERSISTENT · READY"
    @Property(str, notify=changed)
    def lastStateSaveText(self): return self._last_state_save
    @Property(str, notify=changed)
    def lastDecisionReasonText(self): return self._last_decision_reason
    @Property(str, notify=changed)
    def actionableText(self):
        r = self._last_actionable or {}
        if not r:
            return "Kein ACTIONABLE im letzten Scan"
        return f"{r.get('symbol','—')} · Score {float(r.get('score',0)):.1f} · Quality {float(r.get('quality_score',0)):.1f}"
    @Property(str, notify=changed)
    def operationsStatusText(self):
        state = "RUNNING" if self._running else "STOPPED"
        return f"{state} · {self.levelName} · {self.scanStatusText}"
    ...
    def startBot(self):
        ...
        self._apply_interval()
        self.timer.start()
        self._next_scan_at = datetime.now() + timedelta(seconds=int(STRATEGIES[self._level]["scan_seconds"]))
        self._request_scan()
        self.changed.emit()
    ...
    def _apply_scan(self, rows, error):
        ...
        self._next_scan_at = (now + timedelta(seconds=int(STRATEGIES[self._level]["scan_seconds"]))) if self._running else None
        ...
        if error:
            ...
            self._last_decision_reason = "DATA ERROR · neue Trades fail-closed blockiert"
        ...
        actionables = [r for r in valid_rows if r.get("is_actionable") and r.get("signal") == "BUY"]
        self._last_actionable = dict(actionables[0]) if actionables else None
        ...
        for original in self._positions:
            ...
            if should_close:
                ...
                self._trades.append({
                    ...
                    "reason": why, "entry_score": p.get("entry_score"), "quality_score": p.get("quality_score"),
                    "macro_risk": p.get("macro_risk"), "macro_regime": p.get("macro_regime"), "exit_reason": why,
                })
                ...
                p["exit_status"] = "HOLD"
                p["last_check"] = now.strftime("%H:%M:%S")
                survivors.append(p)
        ...
        if len(self._positions) < self._max_positions and self._paper_cash >= self._trade_size:
            ...
            if candidate and not macro_gate.get("allow_new_trade", False):
                ...
                self._last_decision_reason = self._last_action
                self._log(self._last_action)
                candidate = None
                macro_blocked = True
            if candidate:
                ...
                if trade_size <= 0.0:
                    ...
                    pos = {
                        ...
                        "opened": now.strftime("%H:%M:%S"), "opened_at": now.isoformat(timespec="seconds"), "entry_score": candidate["score"],
                        "quality_score": candidate.get("quality_score", 0),
                        "macro_risk": macro_gate.get("risk"), "macro_regime": macro_gate.get("regime"),
                        "news_risk": candidate.get("news_risk", "—"),
                        "stop_price": round(price * (1.0 + float(STRATEGIES[self._level]["stop_pct"]) / 100.0), 4),
                        "take_price": round(price * (1.0 + float(STRATEGIES[self._level]["take_pct"]) / 100.0), 4),
                        "exit_status": "HOLD",
                    }
                    ...
                    self._trades.append({
                        ...
                        "entry_score": candidate.get("score"), "quality_score": candidate.get("quality_score"),
                        "macro_risk": macro_gate.get("risk"), "macro_regime": macro_gate.get("regime"),
                        "news_risk": candidate.get("news_risk", "—"),
