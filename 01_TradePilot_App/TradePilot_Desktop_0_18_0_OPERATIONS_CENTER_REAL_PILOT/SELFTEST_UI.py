...
print("TradePilot Desktop 0.18.0 SELFTEST: OK")
...
assert ("Desktop Pilot 0.15.0" in _qml015) or ("Production REAL Core 0.16.0" in _qml015) or ("REAL Pilot Validation 0.16.1" in _qml015) or ("External REAL Position Validation 0.16.2" in _qml015) or ("10 USD Pilot + Visible Scanner 0.16.3" in _qml015) or ("Economic Calendar Hardening 0.17.1" in _qml015) or ("Multi-Source Economic Calendar 0.17.2" in _qml015) or ("Operations Center & REAL Pilot 0.18.0" in _qml015)
assert "1000 Stocks" in _qml015
assert ("REAL manuell bestätigt" in _qml015) or ("REAL Pilot Readiness integriert" in _qml015) or ("REAL Pilot Readiness + External Position Observer" in _qml015) or ("REAL PILOT READINESS" in _qml015)
...
assert ("REAL Pilot Validation 0.16.1" in qml) or ("External REAL Position Validation 0.16.2" in qml) or ("10 USD Pilot + Visible Scanner 0.16.3" in qml) or ("Economic Calendar Hardening 0.17.1" in qml) or ("Multi-Source Economic Calendar 0.17.2" in qml) or ("Operations Center & REAL Pilot 0.18.0" in qml)
...
assert ('External REAL Position Validation 0.16.2' in qml) or ('10 USD Pilot + Visible Scanner 0.16.3' in qml) or ('Economic Calendar Hardening 0.17.1' in qml) or ('Multi-Source Economic Calendar 0.17.2' in qml) or ('Operations Center & REAL Pilot 0.18.0' in qml)
...
assert ('Economic Calendar Hardening 0.17.1' in _qml17) or ('Multi-Source Economic Calendar 0.17.2' in _qml17) or ('Operations Center & REAL Pilot 0.18.0' in _qml17)
...
# 0.17.2 Multi-Source Economic Calendar
_feed172=(ROOT/'economic_calendar_feed.py').read_text(encoding='utf-8')
_macro172=(ROOT/'macro_logic.py').read_text(encoding='utf-8')
assert 'TRADEPILOT_TRADING_ECONOMICS_KEY' in _feed172
assert 'multi-source-live' in _feed172
assert '_merge_event' in _feed172 and 'field_sources' in _feed172
assert 'Release-Daten noch ausstehend' in _macro172
print('0.17.2 Multi-Source Provider Chain: Xoomar + optional Trading Economics + custom GET: OK')
print('Field-level enrichment: Country/Forecast/Previous/Actual/Unit + provenance: OK')
print('Release Data Pending -> fail-closed: OK')
print('REAL execution POST path: UNCHANGED · REAL AUTO LOCKED/OFF')


# 0.18.0 Operations Center & REAL Pilot
_ops=(ROOT/'operations_center.py').read_text(encoding='utf-8')
_bot18=(ROOT/'bot_engine.py').read_text(encoding='utf-8')
_main18=(ROOT/'main.py').read_text(encoding='utf-8')
_qml18=(ROOT/'qml/Main.qml').read_text(encoding='utf-8')
assert 'REAL_AUTO_LOCKED = True' in _ops
assert 'restart_recovery' in _ops and 'real_audit' in _ops and 'position_monitor' in _ops
assert 'was_running' in _bot18 and 'restartRecoveryText' in _bot18 and 'actionableText' in _bot18
assert 'stop_price' in _bot18 and 'take_price' in _bot18 and 'quality_score' in _bot18
assert 'realAuditJson' in _main18 and 'lastReconcileText' in _main18 and 'tradingModesText' in _main18
assert 'Bot Operations Center' in _qml18 and 'BOT OPERATIONS LOG' in _qml18 and 'REAL EXECUTION AUDIT · STATE ONLY' in _qml18
assert 'Trade History & Execution Audit' in _qml18 and 'REAL AUTO LOCKED' in _qml18
for _f in ['67_TEST_OPERATIONS_CENTER_OFFLINE.py','68_TEST_POSITION_MONITORING_OFFLINE.py','69_TEST_AUDIT_AND_REAL_LOCK_OFFLINE.py','70_TEST_RESTART_PERSISTENCE_OFFLINE.py','71_TEST_OPERATIONS_UI_WIRING_OFFLINE.py']:
    assert (ROOT/_f).exists(), _f
print('0.18.0 Bot Operations Center: OK')
print('Position Monitoring: Entry/Current/P-L/Stop/Take/Strategy/Score/Quality/Macro/News: OK')
print('Trade History + Bot Operations Log + REAL State Audit: OK')
print('Restart Persistence + SHADOW safe resume intent: OK')
print('REAL Readiness + Broker Reconcile UI: OK')
