...
class MacroRiskEngine(QObject):
    ...
    def _apply_refresh(self, snap, error, feed_meta):
        ...
        comp = dict(meta.get('completeness') or {})
        providers = int(meta.get('providers', 0))
        quality = f" · F {comp.get('forecast',0)}% · P {comp.get('previous',0)}%" if comp else ''
        self._feed_status = f'{src} · {providers} Quellen · {count} Events · {horizon} Tage' + quality + (' · STALE' if meta.get('stale') else '')
