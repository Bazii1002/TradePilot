...
ApplicationWindow {
    ...
    visible: true
    color: "#010812"
    title: "TradePilot · " + (backend ? backend.version : "Operations Center & REAL Pilot 0.18.0")
    ...
    Item {
        ...
        Rectangle {
            ...
            Text { x: 84; y: 62; text: "Operations Center & REAL Pilot 0.18.0"; color: "#6f8498"; font.pixelSize: 10; font.family: "Segoe UI" }
        ...
        // PREMIUM VISUAL POLISH II 0.13.4 + FINAL SIGNAL QUALITY GATE 0.14.1: functional dashboard actions + upgraded surfaces/interactions
        // 0.18.0 Operations UI extends the frozen pages; legacy marker: AutoTrader Control · Trade History · Shadow Production Signals
        // FUNCTIONAL PAGES ---------------------------------------------------------
        ...
        Item {
            ...
            NeonCard {
                ...
                Text { x: 20; y: 18; text: "Bot Operations Center"; color: "#f4f7fb"; font.pixelSize: 18; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 49; text: "Dauerbetrieb · Scan-Entscheidungen · Position Monitoring · REAL Readiness · Restart Recovery"; color: "#8da2b6"; font.pixelSize: 11; font.family: "Segoe UI" }
                Rectangle { x: 655; y: 14; width: 405; height: 50; radius: 10; color: macro && macro.risk === "CRITICAL" ? "#32151c" : (macro && macro.risk === "HIGH" ? "#2e2013" : "#071d28"); border.width: 1; border.color: macro && (macro.risk === "HIGH" || macro.risk === "CRITICAL") ? "#b86646" : "#27506b" }
                Text { x: 670; y: 23; text: macro ? ("MACRO  " + macro.regime + " · " + macro.risk) : "MACRO · FAIL-CLOSED"; color: macro && macro.risk === "CRITICAL" ? "#ff7d88" : (macro && macro.risk === "HIGH" ? "#ffb064" : "#8fd2ff"); font.pixelSize: 11; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 670; y: 43; width: 375; text: macro ? macro.botImpactText : "Neue Trades blockiert"; color: "#90a6b8"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
            ...
            NeonCard {
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        Text { x: 122; y: 14; text: modelData.strategy + (modelData.is_actionable ? "  · ACTION" : (modelData.is_finalist ? "  · FINAL" : "")); color: modelData.is_actionable ? "#2de487" : (modelData.is_finalist ? "#c78cff" : "#9d7dff"); font.pixelSize: 11; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                        ...
                        Text { x: 10; y: 40; width: 730; text: "Stop $" + Number(modelData.stop_price || 0).toFixed(2) + " · Take $" + Number(modelData.take_price || 0).toFixed(2) + " · Score " + Number(modelData.entry_score || 0).toFixed(1) + " · Q " + Number(modelData.quality_score || 0).toFixed(1) + " · Macro " + (modelData.macro_risk || "—") + " · " + (modelData.exit_status || "HOLD"); color: "#748a9e"; font.pixelSize: 9; elide: Text.ElideRight; font.family: "Segoe UI" }
            ...
            NeonCard {
                ...
                Text { x: 55; y: 94; text: bot ? ("Stufe " + bot.level + " · " + bot.levelName + " · " + bot.operationsStatusText) : "—"; color: "#8398aa"; font.pixelSize: 10; font.family: "Segoe UI" }
                ...
                Rectangle { x: 20; y: 340; width: 508; height: 16; radius: 8; color: bot && bot.scanBusy ? "#103552" : "#08261f"; border.width: 1; border.color: bot && bot.scanBusy ? "#2aa8ff" : "#1d7452" }
                Text { x: 30; y: 341; width: 488; text: bot ? (bot.scanStatusText + " · " + bot.lastScanSummaryText) : "SCAN BEREIT"; color: bot && bot.scanBusy ? "#8fd2ff" : "#8fe2b9"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }

                Rectangle { x: 20; y: 360; width: 508; height: 150; radius: 10; color: "#071A22"; border.width: 1; border.color: backend && backend.realReadinessReady ? "#1D7452" : "#91384A" }
                Text { x: 36; y: 376; text: "REAL PILOT READINESS"; color: "#DDEAF2"; font.pixelSize: 11; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 36; y: 400; text: backend ? backend.realReadinessText : "—"; color: backend && backend.realReadinessReady ? "#31E28B" : "#FF7F89"; font.pixelSize: 12; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 36; y: 428; text: "Broker"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 128; y: 428; text: backend && backend.brokerConnected ? "CONNECTED" : "OFFLINE"; color: backend && backend.brokerConnected ? "#31E28B" : "#FF7F89"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 258; y: 428; text: "State"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 330; y: 428; text: backend ? backend.realExecutionStateText : "—"; color: "#DDEAF2"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 36; y: 452; text: "Positions"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 128; y: 452; text: backend ? backend.realPositionLimitText : "—"; color: "#DDEAF2"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 258; y: 452; text: "Trades today"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 350; y: 452; text: backend ? backend.realTradesTodayText : "—"; color: "#DDEAF2"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 36; y: 476; text: "Pilot"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 128; y: 476; text: backend ? backend.realPilotAmountText : "$10.00"; color: "#8FD2FF"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 258; y: 476; text: "REAL AUTO"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 350; y: 476; text: "LOCKED"; color: "#FF7F89"; font.pixelSize: 10; font.weight: Font.DemiBold; font.family: "Segoe UI" }

                Text { x: 20; y: 516; text: "ACTIONABLE"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 100; y: 516; width: 420; text: bot ? bot.actionableText : "—"; color: bot && bot.scannerActionable > 0 ? "#31e28b" : "#8da2b5"; font.pixelSize: 9; elide: Text.ElideRight; font.family: "Segoe UI" }
                Text { x: 20; y: 538; text: "Entscheidung"; color: "#71879b"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 100; y: 538; width: 420; text: bot ? bot.lastDecisionReasonText : "—"; color: "#e5edf4"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
                Text { x: 20; y: 560; width: 508; text: bot ? (bot.restartRecoveryText + " · gespeichert " + bot.lastStateSaveText) : "—"; color: "#8fd2ff"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
            }
        }
        ...
        Item {
            ...
            MetricCard { x: 702; y: 0; width: 335; height: 174; title: "Portfolio Value"; onClicked: window.goPage(2); value: backend ? backend.portfolioText : "—"; subtitleLeft: backend && backend.dataFresh ? "eToro REAL · READ ONLY" : "eToro REAL · STALE"; subtitleRight: backend ? backend.portfolioText : "—"; iconSource: Qt.resolvedUrl("../assets/icons/value.svg"); accentColor: "#8d64ff" }
            NeonCard {
                ...
                Text { x: 20; y: 52; text: backend ? backend.brokerStatusText : "—"; color: backend && backend.brokerConnected ? "#31e28b" : "#91a4b7"; font.pixelSize: 16; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 83; width: 290; text: backend ? backend.lastReconcileText : "—"; color: "#7f95a8"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
        ...
        Item {
            ...
            NeonCard {
                ...
                Row {
                    ...
                    ScannerMetric { width: 180; height: 58; label: "RAW BUY"; value: bot ? String(bot.scannerSignals) : "—"; detail: "Finalists"; accentColor: "#ffbe5c" }
                    ScannerMetric { width: 180; height: 58; label: "ACTIONABLE"; value: bot ? String(bot.scannerActionable) : "—"; detail: "Quality Gate · max 3"; accentColor: "#2de487" }
                ...
                Rectangle { x: 20; y: 145; width: 1348; height: 34; radius: 8; color: bot && bot.scanBusy ? "#0a2942" : "#071f1b"; border.width: 1; border.color: bot && bot.scanBusy ? "#2aa8ff" : "#1d7452" }
                Rectangle { x: 32; y: 157; width: 8; height: 8; radius: 4; color: bot && bot.scanBusy ? "#39b6ff" : "#31e28b" }
                Text { x: 52; y: 152; text: bot ? bot.scanStatusText : "SCAN BEREIT"; color: bot && bot.scanBusy ? "#8fd2ff" : "#31e28b"; font.pixelSize: 11; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 290; y: 152; width: 1050; text: bot ? bot.lastScanSummaryText : "Noch kein abgeschlossener Scan"; color: "#9eb0bf"; font.pixelSize: 10; elide: Text.ElideRight; font.family: "Segoe UI" }

                Text { x: 22; y: 194; text: "SYMBOL"; color: "#6f879c"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 170; y: 194; text: "STRATEGY"; color: "#6f879c"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 355; y: 194; text: "SCORE"; color: "#6f879c"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { x: 485; y: 194; text: "REASON"; color: "#6f879c"; font.pixelSize: 9; font.family: "Segoe UI" }
                Text { anchors.right: parent.right; anchors.rightMargin: 34; y: 194; text: "SIGNAL"; color: "#6f879c"; font.pixelSize: 9; font.family: "Segoe UI" }
                Rectangle { x: 20; y: 214; width: 1348; height: 1; color: "#16344c" }
                ListView {
                    x: 20; y: 224; width: 1348; height: 509; clip: true
                    ...
                    delegate: Rectangle {
                        ...
                        Text { x: 150; y: 13; text: modelData.strategy + (modelData.is_actionable ? "  · ACTION" : (modelData.is_finalist ? "  · FINAL" : "")); color: modelData.is_actionable ? "#2de487" : (modelData.is_finalist ? "#c78cff" : "#9d7dff"); font.pixelSize: 11; font.family: "Segoe UI" }
        ...
        Item {
            ...
            NeonCard {
                x: 0; y: 0; width: 1388; height: 132; glow: true; accentColor: macro && macro.risk === "CRITICAL" ? "#ff5969" : (macro && macro.risk === "HIGH" ? "#ff9c52" : "#278fff")
                Text { x: 20; y: 16; text: "Macro Risk Engine"; color: "#f4f7fb"; font.pixelSize: 18; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 47; text: macro ? (macro.regime + " · Makrorisiko " + macro.risk + " · Daten-Sicherheit " + macro.confidence + "%") : "NEUTRAL · HIGH · fail-closed"; color: macro && macro.risk === "CRITICAL" ? "#ff7c87" : (macro && macro.risk === "HIGH" ? "#ffb06a" : "#8fd2ff"); font.pixelSize: 14; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 75; width: 920; text: macro ? macro.nextEventText : "Kalender wird geladen"; color: "#a8bac8"; font.pixelSize: 10; elide: Text.ElideRight; font.family: "Segoe UI" }
                Text { x: 20; y: 98; width: 920; text: macro ? macro.botImpactText : "Neue Trades blockiert, bis Daten sicher sind"; color: macro && macro.allowNewTrade ? "#8fe2b9" : "#ff9b79"; font.pixelSize: 10; font.family: "Segoe UI" }
                Rectangle { x: 1036; y: 27; width: 146; height: 44; radius: 9; color: "#0c3154"; border.width: 1; border.color: "#2a78b6" }
                Text { x: 1036; y: 27; width: 146; height: 44; text: macro && macro.busy ? "LÄDT…" : "AKTUALISIEREN"; color: "#8fd2ff"; font.pixelSize: 10; font.weight: Font.DemiBold; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter; font.family: "Segoe UI" }
                MouseArea { x: 1036; y: 27; width: 146; height: 44; enabled: macro && !macro.busy; cursorShape: Qt.PointingHandCursor; onClicked: if (macro) macro.refreshNow() }
                Text { x: 1200; y: 31; text: "Letztes Update"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 1200; y: 51; width: 160; text: macro ? macro.lastUpdateText : "—"; color: "#c8d5df"; font.pixelSize: 9; elide: Text.ElideRight; font.family: "Segoe UI" }
                Text { x: 1200; y: 78; width: 160; text: macro && macro.lastErrorText ? "FAIL-CLOSED" : "DATA OK"; color: macro && macro.lastErrorText ? "#ff7c87" : "#31e28b"; font.pixelSize: 9; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 1200; y: 99; width: 160; text: macro ? macro.feedStatusText : "Kalender —"; color: "#728ca1"; font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
            }

            NeonCard {
                x: 0; y: 148; width: 1388; height: 616; glow: false; accentColor: "#278fff"
                Text { x: 20; y: 17; text: "Anstehende Veranstaltungen"; color: "#f4f7fb"; font.pixelSize: 17; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 46; text: macro ? ("Forecast · Previous · Actual · Relevanz · Countdown · Economic Surprise   ·   " + macro.upcomingEventCount + " kommende Events") : "Kalender wird geladen"; color: "#8398ab"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 22; y: 79; text: "DATUM / ZEIT"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 150; y: 79; text: "LAND / EVENT"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 650; y: 79; text: "FORECAST"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 765; y: 79; text: "PREVIOUS"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 880; y: 79; text: "ACTUAL"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 995; y: 79; text: "RELEVANZ"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Text { x: 1135; y: 79; text: "COUNTDOWN"; color: "#6f879c"; font.pixelSize: 8; font.family: "Segoe UI" }
                Rectangle { x: 20; y: 99; width: 1348; height: 1; color: "#16344c" }
                ListView {
                    x: 20; y: 108; width: 1348; height: 430; clip: true
                    model: macro ? JSON.parse(macro.eventsJson) : []
                    delegate: Rectangle {
                        width: ListView.view.width; height: 72; radius: 7; color: index % 2 === 0 ? "#071827" : "#061521"
                        Rectangle { x: 0; y: 0; width: 3; height: parent.height; radius: 1.5; color: modelData.relevance === "CRITICAL" ? "#ff5969" : (modelData.relevance === "HIGH" ? "#ff9c52" : (modelData.relevance === "MEDIUM" ? "#e5bf5b" : "#31c982")) }
                        Text { x: 12; y: 12; text: modelData.date_text + "  " + modelData.time_text; color: "#dce6ee"; font.pixelSize: 10; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                        Text { x: 140; y: 10; width: 490; text: modelData.country + " · " + modelData.event_name; color: "#f1f5f8"; font.pixelSize: 11; font.weight: Font.DemiBold; elide: Text.ElideRight; font.family: "Segoe UI" }
                        Text { x: 140; y: 37; width: 490; text: modelData.surprise_text; color: modelData.surprise_direction === "NEGATIVE" ? "#ff858e" : (modelData.surprise_direction === "POSITIVE" ? "#62e7a8" : "#8196aa"); font.pixelSize: 8; elide: Text.ElideRight; font.family: "Segoe UI" }
                        Text { x: 640; y: 20; width: 105; text: modelData.forecast_text; color: "#b7c6d2"; font.pixelSize: 10; font.family: "Segoe UI" }
                        Text { x: 755; y: 20; width: 105; text: modelData.previous_text; color: "#b7c6d2"; font.pixelSize: 10; font.family: "Segoe UI" }
                        Text { x: 870; y: 20; width: 105; text: modelData.actual_text; color: modelData.actual_text === "—" ? "#72889b" : "#eef4f8"; font.pixelSize: 10; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                        Rectangle { x: 980; y: 15; width: 116; height: 28; radius: 7; color: modelData.relevance === "CRITICAL" ? "#34161d" : (modelData.relevance === "HIGH" ? "#362415" : (modelData.relevance === "MEDIUM" ? "#302a15" : "#0b2d24")); border.width: 1; border.color: modelData.relevance === "CRITICAL" ? "#a94450" : (modelData.relevance === "HIGH" ? "#9b6638" : "#365a55") }
                        Text { x: 980; y: 15; width: 116; height: 28; text: modelData.relevance; color: modelData.relevance === "CRITICAL" ? "#ff818b" : (modelData.relevance === "HIGH" ? "#ffb26d" : (modelData.relevance === "MEDIUM" ? "#e9cf70" : "#6bdfa9")); font.pixelSize: 9; font.weight: Font.DemiBold; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter; font.family: "Segoe UI" }
                        Text { x: 1125; y: 20; width: 150; text: modelData.countdown; color: "#8fd2ff"; font.pixelSize: 10; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                    }
                }
                Text { visible: macro && JSON.parse(macro.eventsJson).length === 0; x: 20; y: 180; width: 1348; text: "Keine sicheren Kalenderdaten verfügbar · TradePilot bleibt fail-closed und erhöht das Risiko nicht."; color: "#ff9b79"; font.pixelSize: 11; horizontalAlignment: Text.AlignHCenter; font.family: "Segoe UI" }
                Rectangle { x: 20; y: 550; width: 1348; height: 45; radius: 9; color: "#071922"; border.width: 1; border.color: "#173b52" }
                Text { x: 34; y: 560; width: 1320; text: macro ? ("Bot-Auswirkung: " + macro.botImpactText + " · " + macro.reasonText) : "Bot-Auswirkung: Neue Trades blockiert"; color: macro && macro.allowNewTrade ? "#9cdcbc" : "#ffac83"; font.pixelSize: 9; elide: Text.ElideRight; font.family: "Segoe UI" }
                Text { x: 34; y: 578; width: 1320; text: "Bestehende Positionen: nur überwachen · keine hektischen Macro-Zwangsverkäufe"; color: "#7f95a8"; font.pixelSize: 8; font.family: "Segoe UI" }
            }
        }
        ...
        Item {
            ...
            NeonCard {
                ...
                Text { x: 20; y: 18; text: "Trade History & Execution Audit"; color: "#f4f7fb"; font.pixelSize: 18; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 20; y: 49; text: "Shadow-Entscheidungen bleiben erklärbar; REAL State-Transitions werden separat protokolliert."; color: "#8398ab"; font.pixelSize: 11; font.family: "Segoe UI" }
                ...
                ListView {
                    x: 20; y: 118; width: 1348; height: 405; clip: true
                    ...
                    delegate: Rectangle {
                        ...
                        Text { x: 345; y: 16; text: modelData.strategy + (modelData.is_actionable ? "  · ACTION" : (modelData.is_finalist ? "  · FINAL" : "")); color: modelData.is_actionable ? "#2de487" : (modelData.is_finalist ? "#c78cff" : "#9d7dff"); font.pixelSize: 10; font.family: "Segoe UI" }
                        ...
                        Text { x: 770; y: 16; width: 560; text: (modelData.reason || "") + (modelData.entry_score !== undefined ? (" · Score " + Number(modelData.entry_score || 0).toFixed(1) + " · Q " + Number(modelData.quality_score || 0).toFixed(1) + " · Macro " + (modelData.macro_risk || "—")) : ""); color: "#8398ab"; font.pixelSize: 10; elide: Text.ElideRight; font.family: "Segoe UI" }
                    }
                }
                Rectangle { x: 20; y: 536; width: 1348; height: 1; color: "#16344c" }
                Text { x: 22; y: 551; text: "BOT OPERATIONS LOG"; color: "#6f879c"; font.pixelSize: 9; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                Text { x: 700; y: 551; text: "REAL EXECUTION AUDIT · STATE ONLY"; color: "#6f879c"; font.pixelSize: 9; font.weight: Font.DemiBold; font.family: "Segoe UI" }
                ListView {
                    x: 20; y: 574; width: 650; height: 150; clip: true
                    model: bot ? JSON.parse(bot.eventsJson) : []
                    delegate: Text { width: 640; height: 24; text: modelData.time + "  ·  " + modelData.text; color: "#8ea2b5"; font.pixelSize: 8; elide: Text.ElideRight; verticalAlignment: Text.AlignVCenter; font.family: "Segoe UI" }
                }
                ListView {
                    x: 698; y: 574; width: 670; height: 150; clip: true
                    model: backend ? JSON.parse(backend.realAuditJson) : []
                    delegate: Text { width: 660; height: 24; text: (modelData.ts || "") + "  ·  " + (modelData.event || "STATE") + "  " + (modelData.from_state || "") + " → " + (modelData.to_state || ""); color: "#8ea2b5"; font.pixelSize: 8; elide: Text.ElideRight; verticalAlignment: Text.AlignVCenter; font.family: "Segoe UI" }
                }
        ...
        Item {
            ...
            NeonCard {
                ...
                Text { x: 20; y: 49; text: "TradePilot 0.18.0 · Operations Center · $10 REAL Pilot · REAL AUTO LOCKED"; color: "#8398ab"; font.pixelSize: 11; font.family: "Segoe UI" }
                ...
                Text { x: 42; y: 328; text: "Modes"; color: "#74899d"; font.pixelSize: 10; font.family: "Segoe UI" }
                Text { x: 210; y: 328; text: backend ? backend.tradingModesText : "—"; color: "#f2c45b"; font.pixelSize: 11; font.family: "Segoe UI" }
        ...
        Item {
            ...
            NeonCard {
                ...
                Text { x: 143; y: 108; text: backend ? ("REAL Pilot: " + backend.realReadinessText) : "REAL Pilot"; color: backend && backend.realReadinessReady ? "#31e28b" : "#b4c0cc"; font.pixelSize: 11; font.family: "Segoe UI" }
        ...
        Rectangle {
            ...
            Text { anchors.right: parent.right; anchors.rightMargin: 16; anchors.verticalCenter: parent.verticalCenter; text: "Operations Center & REAL Pilot 0.18.0 · Scanner Optimization · Visual Polish II · 1000 Stocks"; color: "#536d83"; font.pixelSize: 9; font.family: "Segoe UI" }
        }
    }
