...
from production_real_core import ProductionRealCore, REAL_TEST_USD, MAX_REAL_POSITIONS, MAX_TRADES_PER_DAY
from macro_risk_engine import MacroRiskEngine

VERSION = "Operations Center & REAL Pilot 0.18.0"
...
class Backend(QObject):
    ...
    def __init__(self):
        ...
        self._live_status = "External REAL Validation · OBSERVE ONLY · AUTO LOCKED"
        self._live_busy = False
        self._last_reconcile = "Noch kein Broker-Reconcile"
        self.real_core = ProductionRealCore(APP_DIR)
    ...
    @Property(str, notify=dataChanged)
    def lastReconcileText(self): return self._last_reconcile

    @Property(str, notify=dataChanged)
    def realPositionsDetailedJson(self):
        rows = []
        for p in self._positions:
            row = dict(p)
            row.setdefault("symbol", row.get("ticker") or "POSITION")
            row.setdefault("mode", "EXTERNAL / BROKER OBSERVE")
            row.setdefault("exit_status", "OBSERVE ONLY")
            rows.append(row)
        return json.dumps(rows, ensure_ascii=False)

    def _real_audit_rows(self, limit=80):
        path = APP_DIR / "data" / "production_real_audit.jsonl"
        out = []
        if not path.exists():
            return out
        try:
            for line in path.read_text(encoding="utf-8").splitlines()[-limit:]:
                try:
                    ...
                    continue
                # Never surface credentials/payload secrets in Operations UI.
                clean = {k: v for k, v in row.items() if k not in {"api_key", "user_key", "headers", "authorization"}}
                out.append(clean)
        except Exception:
            return []
    ...
    @Property(str, notify=dataChanged)
    def realAuditJson(self): return json.dumps(self._real_audit_rows(), ensure_ascii=False)

    @Property(str, notify=dataChanged)
    def operationsReadinessSummary(self):
        return f"Broker {'CONNECTED' if self._connected else 'OFFLINE'} · {self.realExecutionStateText} · {self.realPositionLimitText} Pos · {self.realTradesTodayText} Trades · REAL AUTO LOCKED"

    @Property(str, constant=True)
    def tradingModesText(self): return "SHADOW · REAL MANUAL · REAL AUTO LOCKED"

    def _real_state(self):
        try:
            return self.real_core.status()
        except Exception:
            return {"state": {"state": "LOCKED"}, "kill_switch": True, "legacy_uncertain_lock": True}

    def _real_trades_today(self):
        path = APP_DIR / "data" / "production_real_audit.jsonl"
        if not path.exists():
            return 0
        ...
        try:
            for line in path.read_text(encoding="utf-8").splitlines():
                ...
                if not str(row.get("ts", "")).startswith(today):
                    continue
                if row.get("event") == "STATE" and row.get("to_state") == "SUBMITTED":
                    key = str(row.get("operation_id") or "")
                    seen.add(key or str(len(seen)))
        except Exception:
            return 0
        return len(seen)

    @Property(str, notify=dataChanged)
    def realExecutionStateText(self):
        return str((self._real_state().get("state") or {}).get("state") or "IDLE")

    @Property(bool, notify=dataChanged)
    def realKillSwitchActive(self):
        return bool(self._real_state().get("kill_switch"))

    @Property(bool, notify=dataChanged)
    def realUncertainLockActive(self):
        st = self._real_state()
        state = str((st.get("state") or {}).get("state") or "")
        return bool(st.get("legacy_uncertain_lock") or state in {"UNCERTAIN", "LOCKED"})

    @Property(str, notify=dataChanged)
    def realPositionLimitText(self):
        return f"{len(self._positions)} / {MAX_REAL_POSITIONS}"

    @Property(str, notify=dataChanged)
    def realTradesTodayText(self):
        return f"{self._real_trades_today()} / {MAX_TRADES_PER_DAY}"

    @Property(str, constant=True)
    def realPilotAmountText(self):
        return f"${REAL_TEST_USD:.2f}"

    @Property(str, constant=True)
    def realAutoStatusText(self):
        return "LOCKED"

    @Property(bool, notify=dataChanged)
    def realReadinessReady(self):
        return bool(self._connected and not self.realKillSwitchActive and not self.realUncertainLockActive and len(self._positions) < MAX_REAL_POSITIONS)

    @Property(str, notify=dataChanged)
    def realReadinessText(self):
        if not self._connected:
            return "BLOCKED · Broker offline"
        if self.realKillSwitchActive:
            return "BLOCKED · Kill Switch"
        if self.realUncertainLockActive:
            return "BLOCKED · Uncertain/Locked"
        if len(self._positions) >= MAX_REAL_POSITIONS:
            return "BLOCKED · Position Limit"
        return "READY · MANUAL PILOT"
    ...
    def prepareLiveBuy(self, symbol, amount_eur):
        ...
        try:
            ...
            self._live_status = "Vorbereitet · Daten prüfen · LIVE startet FINAL-GATE"
            return True
    ...
    def executePreparedLiveBuy(self, confirmation):
        self._set_live_error("0.6.6.6: Echtgeld-POST deaktiviert. Nutze den Readiness-Test 10.")
        return False
    ...
    def _apply_refresh_result(self, snapshot, error):
        ...
        self._status = "eToro REAL · READ ONLY"
        ...
        self._last_reconcile = self._last_refresh + " · Broker GET / state compared"
        self.dataChanged.emit()
...
def main():
    ...
    backend = Backend()
    macro = MacroRiskEngine(APP_DIR)
    bot = BotEngine(APP_DIR, macro)
    macro.changed.connect(bot.changed.emit)
    engine.rootContext().setContextProperty("backend", backend)
    engine.rootContext().setContextProperty("bot", bot)
