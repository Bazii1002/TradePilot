...
VERSION = "1.0 UI Prototype 0.6 · Backend Bridge"
...
class Backend(QObject):
    ...
    def brokerStatusText(self): return self._status
    ...
    @Property(real, notify=dataChanged)
    ...
    def navigationClicked(self, page):
        print(f"[TradePilot 0.6] navigation -> {page}")
    ...
    def _apply_refresh_result(self, snapshot, error):
        ...
        if error:
            self._connected = False
            self._status = "eToro Verbindung fehlgeschlagen"
        ...
        rows = self.history.append(self._equity, self._cash, self._invested)
