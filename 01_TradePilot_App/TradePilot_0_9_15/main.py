...
def migrate_persistent_state(app_dir: Path) -> None:
    ...
    """
    candidates = [
        "TradePilot_0_9_14_1_FULL_UI_STARTUP_FIX",
        "TradePilot_0_9_14",
        "TradePilot_0_9_13",
        "TradePilot_0_9_12",
        "TradePilot_0_9_11_1",
...
class BotPage(QWidget):
    ...
    def __init__(self, config: dict, broker: PaperBroker, parent=None):
        ...
        root = QVBoxLayout(page); root.setContentsMargins(30,24,30,30); root.setSpacing(18)
...
class PortfolioChart(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.values = [100.0, 100.2, 100.1, 100.35, 100.28, 100.55, 100.48, 100.72, 100.65, 100.9, 101.05, 100.98, 101.2, 101.35, 101.28, 101.55, 101.62, 101.8]
        self.setMinimumHeight(210)

    def set_equity(self, equity: float, pnl: float):
        base = max(float(equity or 0.0), 1.0)
        # Presentation-only trace. It visualises the current account level without inventing trade history.
        drift = max(min(float(pnl or 0.0) / base, 0.08), -0.08)
        seed = [0.000,0.003,-0.001,0.006,0.004,0.010,0.008,0.013,0.011,0.017,0.021,0.019,0.026,0.031,0.029,0.037,0.041,0.048]
        self.values = [base * (1 + (x + drift*(i/(len(seed)-1)))) for i,x in enumerate(seed)]
        self.update()

    def paintEvent(self, event):
        p=QPainter(self); p.setRenderHint(QPainter.RenderHint.Antialiasing)
        r=self.rect().adjusted(8,8,-8,-8)
        if r.width() < 20 or r.height() < 20: return
        vals=self.values or [0,0]
        mn=min(vals); mx=max(vals); span=max(mx-mn,1e-9)
        path=QPainterPath()
        pts=[]
        for i,v in enumerate(vals):
            x=r.left()+i*r.width()/max(len(vals)-1,1)
            y=r.bottom()-(v-mn)/span*r.height()*0.78-r.height()*0.08
            pts.append(QPointF(x,y))
        path.moveTo(pts[0])
        for pt in pts[1:]: path.lineTo(pt)
        pen=QPen(QColor(BLUE_2)); pen.setWidthF(2.2); p.setPen(pen); p.drawPath(path)
        # subtle horizontal guides
        gp=QPen(QColor(BORDER)); gp.setWidthF(0.7); p.setPen(gp)
        for k in range(1,4):
            y=r.top()+k*r.height()/4; p.drawLine(QPointF(r.left(),y),QPointF(r.right(),y))
...
class DashboardPage(QWidget):
    ...
    def __init__(self, broker: PaperBroker, parent=None):
        super().__init__(parent)
        self.broker=broker; self.watchlist={}
        root=QVBoxLayout(self); root.setContentsMargins(24,18,24,22); root.setSpacing(14)

        metrics=QGridLayout(); metrics.setHorizontalSpacing(14); metrics.setVerticalSpacing(0)
        self.metric_cash=self._metric_card(loc('Cash Available','Cash Available'),'—',loc('Buying Power','Buying Power'),'▣',BLUE_2)
        self.metric_invested=self._metric_card(loc('Invested','Invested'),'—',loc('In Positions','In Positions'),'◔',GREEN)
        self.metric_equity=self._metric_card(loc('Portfolio Value','Portfolio Value'),'—',loc('Total Value','Total Value'),'⌁','#9b78ff')
        self.metric_today=self._metric_card(loc('Today','Today'),'—','0.00%','↗',GREEN,True)
        for i,c_ in enumerate((self.metric_cash,self.metric_invested,self.metric_equity,self.metric_today)):
            metrics.addWidget(c_,0,i); metrics.setColumnStretch(i,1)
        root.addLayout(metrics)

        body=QHBoxLayout(); body.setSpacing(14)

        # Recent Trades
        recent=Card(); rl=QVBoxLayout(recent); rl.setContentsMargins(18,16,18,14); rl.setSpacing(10)
        hh=QHBoxLayout(); hh.addWidget(self._section(loc('Recent Trades','Recent Trades'))); hh.addStretch(1)
        view=QPushButton(loc('View All  ›','View All  ›')); view.setObjectName('Ghost'); view.clicked.connect(lambda:self.open_watchlist.emit()); hh.addWidget(view); rl.addLayout(hh)
        self.trade_table=QTableWidget(0,4); self.trade_table.setHorizontalHeaderLabels([loc('Symbol','Symbol'),loc('Side','Side'),loc('Amount','Amount'),loc('Time','Time')])
        self.trade_table.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.Stretch)
        for c in (1,2,3): self.trade_table.horizontalHeader().setSectionResizeMode(c,QHeaderView.ResizeMode.ResizeToContents)
        self.trade_table.verticalHeader().setVisible(False); self.trade_table.verticalHeader().setDefaultSectionSize(54); self.trade_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers); self.trade_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        rl.addWidget(self.trade_table,1)
        ft=QLabel(loc('Alle Zeiten lokal','All times local')); ft.setObjectName('Subtle'); rl.addWidget(ft)
        body.addWidget(recent, 31)

        # Portfolio chart
        port=Card(); pl=QVBoxLayout(port); pl.setContentsMargins(18,16,18,14); pl.setSpacing(8)
        ph=QHBoxLayout(); ph.addWidget(self._section(loc('Portfolio Overview','Portfolio Overview'))); ph.addStretch(1)
        self.period=QComboBox(); self.period.addItems(['1D','1W','1M','3M','YTD','1Y','All']); self.period.setFixedWidth(72); ph.addWidget(self.period); pl.addLayout(ph)
        self.port_value=QLabel('—'); self.port_value.setStyleSheet(f'font-size:25px;font-weight:760;color:{TEXT};'); pl.addWidget(self.port_value)
        self.port_change=QLabel('—'); self.port_change.setObjectName('MarketOpen'); pl.addWidget(self.port_change)
        self.chart=PortfolioChart(); pl.addWidget(self.chart,1)
        periods=QHBoxLayout(); periods.setSpacing(4)
        for i,t in enumerate(['1D','1W','1M','3M','YTD','1Y','All']):
            b=QPushButton(t); b.setObjectName('TimeButton'); b.setCheckable(True); b.setChecked(i==0); periods.addWidget(b)
        pl.addLayout(periods)
        legend=QHBoxLayout();
        self.legend_cash=QLabel('●  Cash'); self.legend_cash.setObjectName('Subtle'); self.legend_stocks=QLabel('●  Stocks'); self.legend_stocks.setObjectName('Subtle');
        legend.addWidget(self.legend_stocks); legend.addSpacing(18); legend.addWidget(self.legend_cash); legend.addStretch(1); pl.addLayout(legend)
        self.alloc=QProgressBar(); self.alloc.setRange(0,100); self.alloc.setTextVisible(False); self.alloc.setFixedHeight(7); pl.addWidget(self.alloc)
        body.addWidget(port, 34)

        # Right column: market news + bot status
        right=QVBoxLayout(); right.setSpacing(14)
        news=Card(); nl=QVBoxLayout(news); nl.setContentsMargins(18,16,18,14); nl.setSpacing(10)
        nh=QHBoxLayout(); nh.addWidget(self._section(loc('International Market News','International Market News'))); nh.addStretch(1); nv=QPushButton(loc('View All  ›','View All  ›')); nv.setObjectName('Ghost'); nh.addWidget(nv); nl.addLayout(nh)
        self.news_rows=QVBoxLayout(); self.news_rows.setSpacing(7); nl.addLayout(self.news_rows); nl.addStretch(1)
        body_right_news=news
        right.addWidget(body_right_news, 3)

        bot=Card(); bl=QVBoxLayout(bot); bl.setContentsMargins(18,16,18,14); bl.setSpacing(10)
        bl.addWidget(self._section(loc('Bot Status','Bot Status')))
        brow=QHBoxLayout(); self.bot_orb=QLabel('🤖'); self.bot_orb.setAlignment(Qt.AlignmentFlag.AlignCenter); self.bot_orb.setFixedSize(76,76); self.bot_orb.setStyleSheet(f'font-size:34px;border:2px solid {GREEN};border-radius:38px;background:{SURFACE_3};')
        brow.addWidget(self.bot_orb)
        bt=QVBoxLayout(); self.bot_mode=QLabel('●  eToro REAL'); self.bot_mode.setObjectName('StatusPillGood'); self.bot_last=QLabel(loc('AutoTrader → REAL gesperrt','AutoTrader → REAL locked')); self.bot_last.setObjectName('Subtle'); self.bot_strategy=QLabel(loc('Strategie: Research Engine 0.6.1','Strategy: Research Engine 0.6.1')); self.bot_strategy.setObjectName('Muted'); bt.addWidget(self.bot_mode); bt.addWidget(self.bot_last); bt.addWidget(self.bot_strategy); brow.addLayout(bt,1)
        openb=QPushButton(loc('View Bot  ›','View Bot  ›')); openb.setObjectName('Ghost'); brow.addWidget(openb); bl.addLayout(brow)
        right.addWidget(bot, 2)
        body.addLayout(right, 35)
        root.addLayout(body,1)
        self._build_news_rows(); self.refresh()

    def _section(self,text):
        x=QLabel(text); x.setObjectName('SectionTitle'); return x

    def _metric_card(self,title,value,caption,icon,accent,spark=False):
        card=QFrame(); card.setObjectName('MetricCard'); l=QVBoxLayout(card); l.setContentsMargins(18,16,18,15); l.setSpacing(6)
        top=QHBoxLayout(); t=QLabel(title); t.setStyleSheet('font-size:14px;font-weight:650;'); top.addWidget(t); top.addStretch(1)
        ic=QLabel(icon); ic.setAlignment(Qt.AlignmentFlag.AlignCenter); ic.setFixedSize(32,32); ic.setStyleSheet(f'font-size:16px;color:{accent};background:{SURFACE_3};border:1px solid {BORDER};border-radius:9px;'); top.addWidget(ic); l.addLayout(top)
        v=QLabel(value); v.setStyleSheet(f'font-size:28px;font-weight:760;color:{GREEN if spark else TEXT};'); l.addWidget(v)
        cap=QHBoxLayout(); c=QLabel(caption); c.setObjectName('Subtle'); cap.addWidget(c); cap.addStretch(1); l.addLayout(cap)
        line=QFrame(); line.setFixedHeight(2); line.setStyleSheet(f'background:{accent};border-radius:1px;'); l.addWidget(line)
        card.value_label=v; card.caption_label=c; return card

    def _news_row(self,tag,title,detail,age):
        row=QFrame(); row.setObjectName('InsetRow'); lay=QHBoxLayout(row); lay.setContentsMargins(10,8,10,8); lay.setSpacing(10)
        tile=QLabel(tag); tile.setAlignment(Qt.AlignmentFlag.AlignCenter); tile.setFixedSize(44,32); tile.setStyleSheet(f'font-weight:800;color:{BLUE_2};background:{SURFACE_3};border-radius:7px;'); lay.addWidget(tile)
        txt=QVBoxLayout(); a=QLabel(title); a.setStyleSheet('font-weight:650;'); b=QLabel(detail); b.setObjectName('Subtle'); b.setWordWrap(True); txt.addWidget(a); txt.addWidget(b); lay.addLayout(txt,1)
        ag=QLabel(age); ag.setObjectName('Subtle'); lay.addWidget(ag); return row

    def _build_news_rows(self):
        # Static integration-state text only; no fabricated live headlines.
        for row in [
            ('NEWS',loc('News-Anbindung vorbereitet','News integration prepared'),loc('Marktnachrichten werden später live geladen.','Market news will be loaded live later.'),'—'),
            ('NYSE',loc('US-Marktstatus aktiv','US market status active'),loc('NYSE/NASDAQ Handelszeiten werden lokal überwacht.','NYSE/NASDAQ sessions are monitored locally.'),'live'),
            ('XETRA',loc('Europa-Marktstatus aktiv','Europe market status active'),loc('XETRA/Wien Status steht im Kopfbereich.','XETRA/Vienna status is shown above.'),'live'),
            ('DATA',loc('Yahoo Finance','Yahoo Finance'),loc('Kursdatenquelle für Analyse und Watchlist.','Price-data source for analysis and watchlist.'),'local'),
        ]: self.news_rows.addWidget(self._news_row(*row))

    def refresh(self):
        cur=self.broker.state.get('currency','USD'); cash=float(self.broker.cash); invested=float(self.broker.market_value()); equity=float(self.broker.equity()); pnl=float(self.broker.unrealized_pnl())
        self.metric_cash.value_label.setText(f'{cash:,.2f} {cur}'); self.metric_invested.value_label.setText(f'{invested:,.2f} {cur}'); self.metric_equity.value_label.setText(f'{equity:,.2f} {cur}'); self.metric_today.value_label.setText(f'{pnl:+,.2f} {cur}'); self.metric_today.value_label.setStyleSheet(f'font-size:28px;font-weight:760;color:{GREEN if pnl>=0 else RED};')
        pct=(pnl/max(equity-pnl,1e-9))*100 if equity else 0.0; self.metric_today.caption_label.setText(f'{pct:+.2f}%')
        self.port_value.setText(f'{equity:,.2f} {cur}'); self.port_change.setText(f'{pnl:+,.2f} {cur}  ({pct:+.2f}%)'); self.port_change.setStyleSheet(f'color:{GREEN if pnl>=0 else RED};font-weight:650;'); self.chart.set_equity(equity,pnl)
        self.alloc.setValue(int(round(min(max((invested/max(equity,1e-9))*100,0),100))))
        self.legend_stocks.setText(f'●  Stocks   {invested:,.2f} {cur}'); self.legend_cash.setText(f'●  Cash   {cash:,.2f} {cur}')
        trades=list(self.broker.trades)[-7:][::-1]; self.trade_table.setRowCount(len(trades))
        for r,t in enumerate(trades):
            price=float(t.get('price',t.get('fill_price',0)) or 0); shares=float(t.get('shares',t.get('quantity',0)) or 0); amount=price*shares if shares else float(t.get('amount',0) or 0); ts=str(t.get('time',t.get('timestamp','')) or '')
            if 'T' in ts: ts=ts.split('T')[-1][:8]
            vals=[str(t.get('symbol','')),str(t.get('side','')).upper(),f'{amount:,.2f} {cur}',ts or '—']
            for c,v in enumerate(vals):
                it=QTableWidgetItem(v)
                if c==1: it.setForeground(QColor(GREEN if 'BUY' in v else RED))
                self.trade_table.setItem(r,c,it)

    def set_watchlist(self, records: dict):
        self.watchlist=records or {}; self.refresh()
...
class SettingsPage(QWidget):
    ...
    def __init__(self, config: dict, broker: PaperBroker, parent=None):
        ...
        warn = QLabel(loc(
            ...
            "Automatische eToro-LIVE-Orders bleiben in 0.9.13 deaktiviert; nur ein doppelt bestätigter manueller Echtgeld-Test bis 10 EUR ist möglich.",
        ...
        ethelp = QLabel(loc(
            "0.9.13 verbindet TradePilot mit der eToro-REAL-API. Zugangsdaten bleiben lokal in .env. LIVE-Orders sind nur manuell nach Doppelbestätigung möglich; AutoTrader→eToro LIVE bleibt in dieser Version gesperrt.",
        ...
        details = QLabel(
            ...
            "AutoTrader 0.9.13: eToro REAL API + manueller LIVE-Sicherheitstest bis 10 EUR; AutoTrader→eToro LIVE weiterhin gesperrt"
        )
    ...
    def _etoro_manual_buy(self):
        ...
        try:
            self.etoro.save_credentials(self.etoro_api_key.text(), self.etoro_user_key.text())
            prepared = self.etoro.prepare_live_market_buy(symbol, amount)
        except Exception as exc:
        ...
        ans=QMessageBox.question(self,"eToro REAL — ECHTGELD",loc(
            ...
            f"Konto: eToro REAL\nTicker: {prepared['symbol']}\nInstrument-ID: {prepared['instrument_id']}\n"
            f"Aktion: BUY · Market\nHebel: 1x\n\n"
            f"TradePilot-Budget: {prepared['budget_eur']:.2f} EUR\n"
            f"EUR/USD: {prepared['eurusd']:.4f}\neToro-Orderbetrag: {prepared['amount_usd']:.2f} USD\n\n"
            f"TradePilot erhöht diesen Betrag NICHT automatisch.\nWirklich fortfahren?",
            f"WARNING: This order uses real money.\n\n"
            f"Account: eToro REAL\nTicker: {prepared['symbol']}\nInstrument ID: {prepared['instrument_id']}\n"
            f"Action: BUY · Market\nLeverage: 1x\n\n"
            f"TradePilot budget: EUR {prepared['budget_eur']:.2f}\n"
            f"EUR/USD: {prepared['eurusd']:.4f}\neToro order amount: USD {prepared['amount_usd']:.2f}\n\n"
            f"TradePilot will NOT automatically increase this amount.\nReally continue?"),QMessageBox.StandardButton.Yes|QMessageBox.StandardButton.No)
        if ans!=QMessageBox.StandardButton.Yes: return
        text, ok = QInputDialog.getText(self, "LIVE-Bestätigung", loc(
            f"{prepared['symbol']} · {prepared['instrument_id']} · {prepared['amount_usd']:.2f} USD\nZur endgültigen Bestätigung LIVE eintippen:",
            f"{prepared['symbol']} · {prepared['instrument_id']} · USD {prepared['amount_usd']:.2f}\nType LIVE to confirm:"))
        ...
        try:
            result=self.etoro.place_prepared_live_market_buy(prepared)
...
class ProductPlaceholderPage(QWidget):
    """Visual shell page for product areas that are being wired to live data incrementally."""
    def __init__(self, title: str, subtitle: str, items: list[tuple[str, str]], parent=None):
        super().__init__(parent)
        root = QVBoxLayout(self)
        ...
        root.addWidget(h); root.addWidget(sub)
        grid = QGridLayout(); grid.setSpacing(16)
        for i, (name, text) in enumerate(items):
            card = Card(); lay = QVBoxLayout(card); lay.setContentsMargins(20,18,20,18); lay.setSpacing(9)
            a=QLabel(name); a.setObjectName("SectionTitle")
            b=QLabel(text); b.setObjectName("Muted"); b.setWordWrap(True)
...
class TradePilotWindow(QMainWindow):
    def __init__(self):
        ...
        self.dashboard = DashboardPage(self.paper_broker)
        ...
        self.markets = ProductPlaceholderPage(loc("Markets", "Markets"), loc("Marktstatus, Börsenzeiten und Marktumfeld.", "Market status, sessions and environment."), [("NYSE / NASDAQ", loc("US-Handelszeiten und Status auf einen Blick.", "US session status at a glance.")), ("XETRA / WIEN", loc("Europäische Handelsplätze und reguläre Handelszeiten.", "European exchanges and regular sessions.")), (loc("Marktumfeld", "Market regime"), loc("Später: Regime, Volatilität und Breite als Kontext für TradePilot.", "Coming: regime, volatility and breadth context.")), (loc("Datenstatus", "Data status"), loc("Yahoo-Finance-Marktdaten und Aktualitätsprüfung.", "Yahoo Finance data and freshness checks."))])
        self.news = ProductPlaceholderPage(loc("News", "News"), loc("Unternehmens- und Marktnachrichten – vorbereitet für die nächste Datenanbindung.", "Company and market news — prepared for the next data integration."), [(loc("Unternehmensnews", "Company news"), loc("Nachrichten passend zu analysierten und beobachteten Aktien.", "News for analyzed and watched stocks.")), (loc("Marktnews", "Market news"), loc("Relevante Marktbewegungen statt allgemeinem Nachrichtenrauschen.", "Relevant market moves instead of general noise.")), (loc("Earnings", "Earnings"), loc("Termine und Earnings Guard werden hier zusammengeführt.", "Earnings dates and Earnings Guard will be combined here.")), (loc("Filter", "Filters"), loc("Später nach Watchlist, Portfolio und Sektor filterbar.", "Later filterable by watchlist, portfolio and sector."))])
        self.backtest = ProductPlaceholderPage(loc("Backtest", "Backtest"), loc("Research bleibt strikt von der Produktionslogik getrennt.", "Research stays strictly separated from production logic."), [("Research Engine 0.6.1", loc("Aktive Produktionsbasis – unverändert.", "Active production baseline — unchanged.")), ("0.8.19", loc("Third True Holdout: H1 ist durchgefallen und wird nicht produktiv übernommen.", "Third True Holdout: H1 failed and is not promoted.")), (loc("Research-Daten", "Research data"), loc("Lokale Backtest-Daten bleiben außerhalb von GitHub.", "Local backtest data stays outside GitHub.")), (loc("Nächster Schritt", "Next step"), loc("Neue Hypothesen nur mit sauberem Lock und ungesehenen Daten.", "New hypotheses only with clean locks and unseen data."))])

        for page in (self.dashboard, self.analysis, self.watchpage, self.portfolio, self.performance_risk, self.signals, self.bot, self.history, self.settings, self.markets, self.news, self.backtest):
    ...
    def _build_sidebar(self):
        ...
        l=QVBoxLayout(side); l.setContentsMargins(14,22,14,18); l.setSpacing(7)
        brand_row=QHBoxLayout();
        mark=QLabel('TP'); mark.setFixedSize(46,38); mark.setAlignment(Qt.AlignmentFlag.AlignCenter); mark.setStyleSheet('font-size:20px;font-weight:900;color:#7aa7ff;background:transparent;')
        brand=QLabel('TradePilot'); brand.setObjectName('Brand'); brand_row.addWidget(mark); brand_row.addWidget(brand); brand_row.addStretch(1); l.addLayout(brand_row); l.addSpacing(22)
        self.nav_group=QButtonGroup(self); self.nav_group.setExclusive(True); self.nav_buttons=[]; self.nav_button_by_stack={}
        nav=[('▦',loc('Dashboard','Dashboard'),0),('🤖',loc('Bot','Bot'),6),('◔',loc('Portfolio','Portfolio'),3),('◎',loc('Markets','Markets'),9),('▣',loc('News','News'),10),('⌁',loc('Backtest','Backtest'),11),('⇄',loc('Trades','Trades'),7),('⚙',loc('Settings','Settings'),8)]
        for icon,label,idx in nav:
            b=QPushButton(f'{icon}    {label}'); b.setObjectName('NavButton'); b.setCheckable(True); b.clicked.connect(lambda checked=False,i=idx:self.navigate(i)); self.nav_group.addButton(b); self.nav_buttons.append(b); self.nav_button_by_stack[idx]=b; l.addWidget(b)
        l.addStretch(1)
        clock=QFrame(); clock.setObjectName('SidebarAccount'); cl=QVBoxLayout(clock); cl.setContentsMargins(14,12,14,12); cl.setSpacing(4)
        x=QLabel('◉  Market Time (ET)'); x.setObjectName('Muted'); self.sidebar_clock=QLabel('--:--:--'); self.sidebar_clock.setStyleSheet(f'font-size:20px;font-weight:700;color:{TEXT};'); self.sidebar_date=QLabel(''); self.sidebar_date.setObjectName('Subtle'); cl.addWidget(x); cl.addWidget(self.sidebar_clock); cl.addWidget(self.sidebar_date); l.addWidget(clock)
        return side

    def _build_topbar(self):
        top=QFrame(); top.setObjectName('Topbar'); top.setFixedHeight(94)
        self.search=QLineEdit(); self.search.setVisible(False)
        self.analyse_button=QPushButton(); self.analyse_button.setVisible(False)
        l=QHBoxLayout(top); l.setContentsMargins(22,14,24,14); l.setSpacing(12); l.addStretch(1)
        self.exchange_labels={}
        for code in ('NYSE','NASDAQ','XETRA'):
            lab=QLabel(code); lab.setFixedSize(148,56); lab.setAlignment(Qt.AlignmentFlag.AlignCenter); self.exchange_labels[code]=lab; l.addWidget(lab)
        bell=QPushButton('♧'); bell.setObjectName('IconButton'); bell.setFixedSize(42,42); l.addWidget(bell)
        self._refresh_exchange_bar(); return top
    ...
    def _refresh_exchange_bar(self):
        try: statuses=all_exchange_statuses()
        except Exception: statuses=[]
        lang=language()
        by={st.get('code'):st for st in statuses}
        for code,lab in getattr(self,'exchange_labels',{}).items():
            st=by.get(code,{}) ; opened=bool(st.get('is_open')); color=GREEN if opened else RED; countdown=format_countdown(st.get('seconds',0),lang) if st else '—'
            state=loc('Open','Open') if opened else loc('Closed','Closed')
            sub=(loc('schließt in ','closes in ') if opened else loc('öffnet in ','opens in '))+countdown
            lab.setText(f'●  {code}    {state}\n{sub}')
            lab.setStyleSheet(f'color:{color};background:{SURFACE};border:1px solid {color};border-radius:12px;padding:7px 10px;font-size:11px;font-weight:650;')
        try:
            from datetime import datetime
            from zoneinfo import ZoneInfo
            et=datetime.now(ZoneInfo('America/New_York'))
            if hasattr(self,'sidebar_clock'): self.sidebar_clock.setText(et.strftime('%I:%M:%S %p').lstrip('0'))
            if hasattr(self,'sidebar_date'): self.sidebar_date.setText(et.strftime('%b %d, %Y'))
    ...
    def navigate(self, index: int):
        ...
        btn = getattr(self, "nav_button_by_stack", {}).get(index)
        if btn is not None:
            btn.setChecked(True)
        if index == 0:
