...
print('='*108)
print('TRADEPILOT 0.16.3 - BROKER REAL PILOT READINESS / USD 10 / READ ONLY / HARD NO POST')
print('='*108)
...
if not client.has_credentials():
    ...
    raise SystemExit(0)
s=client.snapshot(); positions=list(s.get('positions') or [])
st=core.status(); state=(st.get('state') or {}).get('state','IDLE')
kill=bool(st.get('kill_switch')); uncertain=bool(st.get('legacy_uncertain_lock') or state in {'UNCERTAIN','LOCKED'})
