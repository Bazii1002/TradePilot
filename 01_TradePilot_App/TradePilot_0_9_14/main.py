...
class DashboardPage(QWidget):
    """TradePilot 0.9.13 dashboard matching the frozen product design language.

    The dashboard remains a presentation layer over the existing local paper
    account and watchlist. No trading rules are changed here.
    """
    ...
    def __init__(self, broker: PaperBroker, parent=None):
        ...
        title = QLabel(loc("Dashboard", "Dashboard")); title.setObjectName("PageTitle")
        subtitle = QLabel(loc("Dein Portfolio, aktuelle Trades und TradePilot-Status auf einen Blick.", "Your portfolio, current trades and TradePilot status at a glance.")); subtitle.setObjectName("Muted")
        hleft.addWidget(title); hleft.addWidget(subtitle)
        head.addLayout(hleft); head.addStretch(1)
        live = QLabel(loc("●  SYSTEM BEREIT", "●  SYSTEM READY")); live.setObjectName("StatusPillGood")
        head.addWidget(live)
        root.addLayout(head)

        # Frozen product KPI row: Cash / Invested / Portfolio / Today
        metrics = QGridLayout(); metrics.setSpacing(12)
        self.metric_cash = self._metric_card("$", loc("CASH AVAILABLE", "CASH AVAILABLE"), "—", loc("verfügbares Kapital", "available capital"), BLUE_2)
        self.metric_invested = self._metric_card("↗", loc("INVESTED", "INVESTED"), "—", loc("aktuell investiert", "currently invested"), YELLOW)
        self.metric_equity = self._metric_card("◉", loc("PORTFOLIO VALUE", "PORTFOLIO VALUE"), "—", loc("Cash + Positionen", "cash + positions"), GREEN)
        self.metric_today = self._metric_card("±", loc("TODAY", "TODAY"), "—", loc("unrealisierter P/L", "unrealized P/L"), GREEN)
        for i, card in enumerate((self.metric_cash, self.metric_invested, self.metric_equity, self.metric_today)):
            metrics.addWidget(card, 0, i); metrics.setColumnStretch(i, 1)
        root.addLayout(metrics)

        center = QHBoxLayout(); center.setSpacing(14)

        # Recent trades
        recent = Card(); rl = QVBoxLayout(recent); rl.setContentsMargins(18, 16, 18, 16); rl.setSpacing(10)
        rh = QHBoxLayout(); rh.addWidget(self._section(loc("Recent Trades", "Recent Trades"))); rh.addStretch(1)
        badge = QLabel(loc("LOKAL", "LOCAL")); badge.setObjectName("StatusPillNeutral"); rh.addWidget(badge); rl.addLayout(rh)
        self.trade_table = QTableWidget(0, 5)
        self.trade_table.setHorizontalHeaderLabels([loc("SYMBOL","SYMBOL"), loc("SEITE","SIDE"), loc("PREIS","PRICE"), loc("P/L","P/L"), loc("STATUS","STATUS")])
        self.trade_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.trade_table.verticalHeader().setVisible(False); self.trade_table.verticalHeader().setDefaultSectionSize(40)
        ...
        center.addWidget(recent, 5)

        # Portfolio overview
        portfolio = Card(); pl = QVBoxLayout(portfolio); pl.setContentsMargins(18,16,18,16); pl.setSpacing(10)
        ph = QHBoxLayout(); ph.addWidget(self._section(loc("Portfolio Overview", "Portfolio Overview"))); ph.addStretch(1)
        self.position_count = QLabel("0"); self.position_count.setObjectName("StatusPillNeutral"); ph.addWidget(self.position_count); pl.addLayout(ph)
        self.position_table = QTableWidget(0, 4)
        self.position_table.setHorizontalHeaderLabels([loc("SYMBOL","SYMBOL"), loc("WERT","VALUE"), loc("P/L","P/L"), loc("ANTEIL","WEIGHT")])
        self.position_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.position_table.verticalHeader().setVisible(False); self.position_table.verticalHeader().setDefaultSectionSize(40)
        self.position_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        pl.addWidget(self.position_table, 1)
        center.addWidget(portfolio, 4)
        root.addLayout(center, 5)

        lower = QHBoxLayout(); lower.setSpacing(14)
        # Watchlist / opportunities
        watch = Card(); wl=QVBoxLayout(watch); wl.setContentsMargins(18,16,18,16); wl.setSpacing(10)
        wh=QHBoxLayout(); wh.addWidget(self._section(loc("Watchlist", "Watchlist"))); wh.addStretch(1)
        btn=QPushButton(loc("Alle anzeigen  →", "View all  →")); btn.setObjectName("Ghost"); btn.clicked.connect(self.open_watchlist.emit); wh.addWidget(btn); wl.addLayout(wh)
        self.watch_table=QTableWidget(0,4); self.watch_table.setHorizontalHeaderLabels(["SYMBOL",loc("U-SCORE","C-SCORE"),loc("EINSTIEG","ENTRY"),loc("STATUS","STATUS")])
        self.watch_table.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.Stretch); self.watch_table.horizontalHeader().setSectionResizeMode(3,QHeaderView.ResizeMode.Stretch)
        self.watch_table.verticalHeader().setVisible(False); self.watch_table.verticalHeader().setDefaultSectionSize(38); self.watch_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        self.watch_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows); self.watch_table.doubleClicked.connect(self._double_click)
        wl.addWidget(self.watch_table,1); lower.addWidget(watch,5)

        status=Card(); sl=QVBoxLayout(status); sl.setContentsMargins(18,16,18,16); sl.setSpacing(12)
        sl.addWidget(self._section(loc("TradePilot Status", "TradePilot Status")))
        self.status_lines=QVBoxLayout(); self.status_lines.setSpacing(8); sl.addLayout(self.status_lines); sl.addStretch(1)
        lower.addWidget(status,4)
        root.addLayout(lower,4)
        self.refresh()

    def _metric_card(self, icon: str, title: str, value: str, caption: str, accent: str):
        card=QFrame(); card.setObjectName("MetricCard")
        l=QVBoxLayout(card); l.setContentsMargins(18,15,18,15); l.setSpacing(5)
        top=QHBoxLayout(); icon_lbl=QLabel(icon); icon_lbl.setFixedSize(30,30); icon_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter); icon_lbl.setStyleSheet(f"font-size:15px;color:{accent};background:{SURFACE_3};border:1px solid {BORDER};border-radius:9px;font-weight:800;")
        title_lbl=QLabel(title); title_lbl.setObjectName("Eyebrow"); top.addWidget(icon_lbl); top.addWidget(title_lbl); top.addStretch(1); l.addLayout(top)
        b=QLabel(value); b.setStyleSheet(f"font-size:29px;font-weight:760;color:{TEXT};"); c=QLabel(caption); c.setObjectName("Subtle")
        l.addWidget(b); l.addWidget(c); line=QFrame(); line.setFixedHeight(3); line.setStyleSheet(f"background:{accent};border-radius:1px;"); l.addWidget(line)
        card.value_label=b; card.caption_label=c; return card

    def _section(self,text:str):
        x=QLabel(text); x.setObjectName("SectionTitle"); return x

    def _status_row(self, dot: str, title: str, detail: str, color: str):
        row=QFrame(); row.setObjectName("InsetRow"); lay=QHBoxLayout(row); lay.setContentsMargins(11,9,11,9)
        d=QLabel(dot); d.setStyleSheet(f"color:{color};font-size:12px;"); lay.addWidget(d)
        txt=QVBoxLayout(); a=QLabel(title); a.setStyleSheet("font-weight:650;"); b=QLabel(detail); b.setObjectName("Subtle"); txt.addWidget(a); txt.addWidget(b); lay.addLayout(txt,1); return row

    def refresh(self):
        cur=self.broker.state.get("currency","USD")
        cash=float(self.broker.cash); invested=float(self.broker.market_value()); equity=float(self.broker.equity()); pnl=float(self.broker.unrealized_pnl())
        self.metric_cash.value_label.setText(f"{cash:,.2f} {cur}")
        self.metric_invested.value_label.setText(f"{invested:,.2f} {cur}")
        self.metric_equity.value_label.setText(f"{equity:,.2f} {cur}")
        self.metric_today.value_label.setText(f"{pnl:+,.2f} {cur}")
        self.metric_today.value_label.setStyleSheet(f"font-size:29px;font-weight:760;color:{GREEN if pnl>=0 else RED};")

        trades=list(self.broker.trades)[-6:][::-1]; self.trade_table.setRowCount(len(trades))
        for r,t in enumerate(trades):
            vals=[str(t.get("symbol","")), str(t.get("side","")), f"{float(t.get('price',t.get('fill_price',0)) or 0):,.2f}", f"{float(t.get('pnl',0) or 0):+,.2f}", str(t.get("status","FILLED"))]
            for c_,v in enumerate(vals):
                it=QTableWidgetItem(v)
                if c_==0: it.setForeground(QColor(BLUE_2))
                if c_==3: it.setForeground(QColor(GREEN if float(t.get('pnl',0) or 0)>=0 else RED))
                self.trade_table.setItem(r,c_,it)

        positions=list(self.broker.positions.values()); self.position_count.setText(loc(f"{len(positions)} offen",f"{len(positions)} open")); self.position_table.setRowCount(len(positions))
        total=max(invested,0.0001)
        for r,pos in enumerate(positions[:8]):
            shares=float(pos.get("shares",0) or 0); entry=float(pos.get("entry_price",0) or 0); last=float(pos.get("last_price",entry) or entry); value=shares*last; pp=shares*(last-entry); weight=value/total*100
            vals=[str(pos.get("symbol","")),f"{value:,.2f}",f"{pp:+,.2f}",f"{weight:.1f}%"]
            for c_,v in enumerate(vals):
                it=QTableWidgetItem(v)
                if c_==0: it.setForeground(QColor(BLUE_2))
                if c_==2: it.setForeground(QColor(GREEN if pp>=0 else RED))
                self.position_table.setItem(r,c_,it)

        # Status panel
        while self.status_lines.count():
            item=self.status_lines.takeAt(0); w=item.widget();
            if w: w.deleteLater()
        self.status_lines.addWidget(self._status_row("●",loc("Research Engine 0.6.1","Research Engine 0.6.1"),loc("Produktionslogik unverändert","production logic unchanged"),GREEN))
        self.status_lines.addWidget(self._status_row("●",loc("eToro REAL","eToro REAL"),loc("manueller Echtgeld-Test · max. 10 €","manual live test · max €10"),YELLOW))
        self.status_lines.addWidget(self._status_row("●",loc("AutoTrader → REAL gesperrt","AutoTrader → REAL locked"),loc("kein automatischer Echtgeldversand","no automatic live execution"),BLUE_2))

    def set_watchlist(self, watchlist: dict):
        self.watchlist=watchlist or {}; records=list(self.watchlist.values()); records.sort(key=lambda r:(r.get("einstieg_score") is not None,r.get("einstieg_score") or -1),reverse=True)
        top=records[:6]; self.watch_table.setRowCount(len(top))
        for row,r in enumerate(top):
            vals=[r.get("symbol",""),f"{r.get('unternehmensscore','—')}/100",f"{r.get('einstieg_score','—')}/100",r.get("status","")]
            for col,val in enumerate(vals):
                it=QTableWidgetItem(str(val));
                if col==0: it.setForeground(QColor(BLUE_2))
                if col==3: it.setForeground(QColor(status_color(str(val))))
                self.watch_table.setItem(row,col,it)
        self.refresh()
    ...
    def _double_click(self):
        ...
        if row>=0:
            ...
            if item: self.open_analysis.emit(item.text())
...
class TradePilotWindow(QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        l = QVBoxLayout(side)
        l.setContentsMargins(16, 20, 16, 18)
        l.setSpacing(6)

        brand_row = QHBoxLayout()
        mark = QFrame(); mark.setFixedSize(38, 38); mark.setObjectName("BrandMark")
        ml = QVBoxLayout(mark); ml.setContentsMargins(0,0,0,0)
        logo = QLabel("▥"); logo.setAlignment(Qt.AlignmentFlag.AlignCenter); logo.setObjectName("BrandGlyph"); ml.addWidget(logo)
        brand = QLabel("TradePilot"); brand.setObjectName("Brand")
        brand_row.addWidget(mark); brand_row.addWidget(brand); brand_row.addStretch(1)
        l.addLayout(brand_row)
        version = QLabel(f"Desktop  ·  {VERSION}"); version.setObjectName("Subtle"); l.addWidget(version)
        l.addSpacing(22)

        self.nav_group = QButtonGroup(self); self.nav_group.setExclusive(True)
        self.nav_buttons = []
        self.nav_button_by_stack = {}
        nav = [
            ("⌂", loc("Dashboard","Dashboard"), 0),
            ("◆", loc("Bot","Bot"), 6),
            ("◔", loc("Portfolio","Portfolio"), 3),
            ("⌁", loc("Markets","Markets"), 9),
            ("◈", loc("News","News"), 10),
            ("◫", loc("Backtest","Backtest"), 11),
            ("↺", loc("Trades","Trades"), 7),
            ("⚙", loc("Settings","Settings"), 8),
        ]
        for icon, label, idx in nav:
            b=QPushButton(f"{icon}    {label}"); b.setObjectName("NavButton"); b.setCheckable(True)
            b.clicked.connect(lambda checked=False, i=idx: self.navigate(i))
            self.nav_group.addButton(b); self.nav_buttons.append(b); self.nav_button_by_stack[idx]=b; l.addWidget(b)
        l.addStretch(1)

        account = QFrame(); account.setObjectName("SidebarAccount")
        al=QVBoxLayout(account); al.setContentsMargins(14,12,14,12); al.setSpacing(4)
        a=QLabel("●  eToro REAL"); a.setObjectName("SidebarAccountTitle")
        b=QLabel(loc("Manueller Live-Test · max. 10 €", "Manual live test · max €10")); b.setObjectName("Subtle")
        c=QLabel(loc("AutoTrader → REAL gesperrt", "AutoTrader → REAL locked")); c.setObjectName("SidebarAccountLock")
        al.addWidget(a); al.addWidget(b); al.addWidget(c); l.addWidget(account)
        return side

    def _build_topbar(self):
        top = QFrame(); top.setObjectName("Topbar"); top.setFixedHeight(78)
        l=QHBoxLayout(top); l.setContentsMargins(30,14,30,14); l.setSpacing(12)
        self.search=QLineEdit(); self.search.setObjectName("GlobalSearch"); self.search.setPlaceholderText(loc("Ticker eingeben, z. B. AAPL, JPM, XOM", "Enter ticker, e.g. AAPL, JPM, XOM")); self.search.setMinimumWidth(390); self.search.setMaximumWidth(560); self.search.returnPressed.connect(self._search_clicked); l.addWidget(self.search)
        self.analyse_button=QPushButton(loc("Analyse starten   ▶", "Start analysis   ▶")); self.analyse_button.setObjectName("Primary"); self.analyse_button.clicked.connect(self._search_clicked); l.addWidget(self.analyse_button)
        l.addStretch(1)
        market=QVBoxLayout(); market.setSpacing(0)
        self.top_status_dot=QLabel("●  " + loc("Marktstatus", "Market status")); self.top_status_dot.setObjectName("MarketOpen")
        self.top_status=QLabel(loc("Bereit", "Ready")); self.top_status.setObjectName("Subtle")
        market.addWidget(self.top_status_dot); market.addWidget(self.top_status); l.addLayout(market)
        bell=QPushButton("♢"); bell.setObjectName("IconButton"); bell.setFixedSize(38,38); l.addWidget(bell)
        helpb=QPushButton("?"); helpb.setObjectName("IconButton"); helpb.setFixedSize(38,38); l.addWidget(helpb)
        avatar=QLabel("TP"); avatar.setObjectName("Avatar"); avatar.setFixedSize(40,40); avatar.setAlignment(Qt.AlignmentFlag.AlignCenter); l.addWidget(avatar)
        return top
