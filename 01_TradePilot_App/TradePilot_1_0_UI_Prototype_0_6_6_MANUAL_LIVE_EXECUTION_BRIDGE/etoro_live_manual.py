...
class EtoroManualLiveBroker:
    ...
    def credentials(self) -> tuple[str, str]:
        ...
        api = os.getenv("ETORO_API_KEY", "").strip() or env.get("ETORO_API_KEY", "").strip()
        user = os.getenv("ETORO_REAL_USER_KEY", "").strip() or env.get("ETORO_REAL_USER_KEY", "").strip()
        return api, user

    def _headers(self) -> dict[str,str]:
        api,user=self.credentials()
        if not api or not user:
            raise EtoroLiveError("eToro-REAL-Zugangsdaten fehlen.")
        return {
            "x-api-key": api,
            "x-user-key": user,
    ...
    def search_exact_instrument(self, symbol: str) -> dict:
        ...
        resp=self.session.get(f"{BASE_V1}/market-data/search", headers=self._headers(), params={"search":symbol}, timeout=self.timeout)
        rows=self._instrument_rows(self._decode(resp))
        exact=[]
        for row in rows:
            s=str(row.get("symbol") or row.get("ticker") or row.get("internalSymbol") or "").strip().upper()
            if s==symbol:
                exact.append(row)
        if not exact:
