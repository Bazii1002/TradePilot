...
"""Read-only eToro REAL adapter for TradePilot 1.0 UI Prototype 0.6.4.
...
def _position_pnl(position: dict) -> float | None:
    direct = _pick_number(position, ("pnl", "profitLoss", "profit", "unrealizedPnl", "openPnl"))
    if direct is not None:
        return direct
    # eToro P/L payloads may expose unrealizedPnL as a nested object,
    # e.g. {"unrealizedPnL": {"pnL": 12.34}}. Keep the search scoped
    # to the position so we never mix account-level values into a row.
    nested = position.get("unrealizedPnL") or position.get("unrealizedPnl")
    if isinstance(nested, dict):
        return _pick_number_recursive(nested, ("pnl", "profitLoss", "profit", "value"), max_depth=2)
    return None
...
def parse_snapshot(portfolio_payload: Any, pnl_payload: Any | None = None) -> dict:
    ...
    # 0.6.4 maps the concrete ClientPortfolio schema observed on the REAL account.
    # credit = available trading balance; accountCurrencyId 1 = USD.
    account_currency_id = _pick_number_recursive(pnl_data or data, ("accountCurrencyId",), max_depth=2)
    currency = _pick_text_recursive(data, ("currency", "accountCurrency", "baseCurrency"))
    if not currency and account_currency_id == 1:
        currency = "USD"
    currency = (currency or "USD").upper()

    cash = _pick_number_recursive(data, (
        "credit", "buyingPower", "cash", "availableCash", "cashAvailable",
        "availableBalance", "cashBalance", "balance"
    ))
    bonus_credit = _pick_number_recursive(data, ("bonusCredit",))

    # The P/L endpoint is authoritative for account-level unrealized P/L when present.
    open_pnl = _pick_number_recursive(pnl_data, ("unrealizedPnL", "openPnl", "pnl"), max_depth=2)
    if open_pnl is None:
        open_pnls = [v for v in (_position_pnl(p) for p in positions) if v is not None]
        if open_pnls:
            open_pnl = sum(open_pnls)
        else:
            open_pnl = _pick_number_recursive(data, ("unrealizedPnL", "openPnl", "pnl"), max_depth=2)

    # Preserve explicit fields on API variants that provide them. The observed REAL
    # ClientPortfolio does not, but older/documented variants may expose them directly.
    direct_invested = _pick_number_recursive(data, (
        "invested", "investedAmount", "positionsValue", "marketValue", "openPositionsValue"
    ))
    direct_equity = _pick_number_recursive(data, (
        "equity", "portfolioValue", "netEquity", "totalValue", "totalEquity", "accountValue"
    ))

    # eToro's position `amount` is the invested amount/margin. An explicitly empty
    # positions array means invested is exactly 0 rather than unknown.
    position_values = [v for v in (_position_value(p) for p in positions) if v is not None]
    if direct_invested is not None:
        invested = direct_invested
    elif positions:
        invested = sum(position_values) if position_values else None
    else:
        invested = 0.0

    # Prefer an explicit equity/portfolio field where available. Otherwise build a
    # conservative account value from available cash + invested amount + unrealized P/L.
    # Bonus credit stays separate and is not silently counted as cash/equity.
    equity = direct_equity
    if equity is None and cash is not None and invested is not None:
        equity = cash + invested + (open_pnl or 0.0)

    # The endpoint exposes unrealized P/L, not necessarily today's realized+unrealized
    # performance. Keep Today unknown unless an explicit daily field exists.
    ...
    return {
        "currency": currency,
        "account_currency_id": int(account_currency_id) if account_currency_id is not None else None,
        "cash": cash,
