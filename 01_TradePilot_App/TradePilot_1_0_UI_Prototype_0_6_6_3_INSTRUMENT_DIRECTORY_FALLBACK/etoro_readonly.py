...
"""Read-only eToro REAL adapter for TradePilot 1.0 UI Prototype 0.6.5.
...
def write_credentials(path: Path, public_key: str, private_key: str) -> None:
    public_key = str(public_key or "").strip().replace("\n", "").replace("\r", "")
    private_key = str(private_key or "").strip().replace("\n", "").replace("\r", "")
    if not public_key or not private_key:
        raise EtoroReadOnlyError("Öffentlicher Key (API Key) und Privater Key (User-Key) dürfen nicht leer sein.")
    text = (
        "# TradePilot local eToro REAL credentials - DO NOT COMMIT\n"
        "ETORO_ENV=real\n"
        f"ETORO_PUBLIC_KEY={public_key}\n"
        f"ETORO_PRIVATE_KEY={private_key}\n"
    )
...
def parse_snapshot(portfolio_payload: Any, pnl_payload: Any | None = None) -> dict:
    ...
    # Observed eToro REAL clientPortfolio fields (0.6.4/0.6.5):
    # credit, bonusCredit, positions and, on the P/L surface, accountCurrencyId
    # plus unrealizedPnL. Generic aliases remain as fallbacks for API variants.
    account_currency_id = _pick_number_recursive(pnl_data, ("accountCurrencyId",))
    if account_currency_id is None:
        account_currency_id = _pick_number_recursive(data, ("accountCurrencyId",))
    currency_id_map = {1: "USD", 2: "EUR", 3: "GBP"}
    currency = _pick_text_recursive(data, ("currency", "accountCurrency", "baseCurrency"))
    if not currency:
        currency = _pick_text_recursive(pnl_data, ("currency", "accountCurrency", "baseCurrency"))
    if not currency and account_currency_id is not None:
        currency = currency_id_map.get(int(account_currency_id))
    ...
    if bonus_credit is None:
        bonus_credit = _pick_number_recursive(pnl_data, ("bonusCredit",))
    ...
    if invested is None:
        ...
        elif isinstance(data.get("positions"), list) and len(data.get("positions")) == 0:
            # An explicit empty positions array is strong evidence of zero invested value.
            invested = 0.0

    open_pnls = [v for v in (_position_pnl(p) for p in positions) if v is not None]
    open_pnl = sum(open_pnls) if open_pnls else _pick_number_recursive(
        pnl_data, ("unrealizedPnL", "unrealizedPnl", "openPnl", "pnl")
    )
    if open_pnl is None:
        open_pnl = _pick_number_recursive(data, ("unrealizedPnL", "unrealizedPnl", "openPnl", "pnl"))

    # Conservative equity mapping: explicit API equity wins. Otherwise cash plus
    # current position values. For an explicitly empty portfolio, equity==cash.
    ...
    return {
        ...
        "equity": equity,
        "open_pnl": open_pnl,
        "account_currency_id": None if account_currency_id is None else int(account_currency_id),
        "today_pnl": today_pnl,
        "today_pct": today_pct,
        "positions": rows,
...
class EtoroReadOnlyClient:
    ...
    def credentials(self) -> tuple[str, str]:
        ...
        api_key = (os.getenv("ETORO_PUBLIC_KEY", "").strip() or env.get("ETORO_PUBLIC_KEY", "").strip()
                   or os.getenv("ETORO_API_KEY", "").strip() or env.get("ETORO_API_KEY", "").strip())
        user_key = (os.getenv("ETORO_PRIVATE_KEY", "").strip() or env.get("ETORO_PRIVATE_KEY", "").strip()
                    or os.getenv("ETORO_USER_KEY", "").strip() or env.get("ETORO_USER_KEY", "").strip()
