...
VERSION = "0.8.18 THIRD HOLDOUT LOCK + UNIVERSE"
...
def main():
    ...
    if symbol_hash(first) != EXPECTED_FIRST_HOLDOUT_HASH:
        raise RuntimeError("STOP: First-Holdout-Hash stimmt nicht mit 0.8.12.1 ueberein.")
    if symbol_hash(second) != EXPECTED_SECOND_HOLDOUT_HASH:
        raise RuntimeError("STOP: Second-Holdout-Hash stimmt nicht mit 0.8.14 ueberein.")

    previous_all = discovery | first | second
    print(f"Einzigartige bereits verwendete:     {len(previous_all)}")
    print("First-/Second-Holdout-Hashes:         OK\n")
